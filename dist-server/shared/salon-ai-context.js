...
export const SALON_SYSTEM = 'Bạn là chuyên viên tư vấn khách hàng tại salon tóc (xưng em/con theo ngữ cảnh). Trả lời ngắn gọn, lịch sự, tiếng Việt. ' +
...
export function buildFanpagePrompt(branch) {
    return [
...
export const IMAGE_SAMPLE_LEGACY_KEYS = {
    noi_long_vu_den_chum_den: 'noi_long_vu_den',
};
export const IMAGE_SAMPLE_ALIASES = {
...
export function shouldBlockImageSampleSend(...textParts) {
    const normalized = normalizeSearchText(textParts.filter(Boolean).join('\n'));
    if (!normalized)
        return false;
    if (/(khong gui them anh|khong gui anh nua|se khong gui.*anh|dung gui anh|thoi gui anh|xin loi.*gui anh|lam phien)/.test(normalized)) {
        return true;
    }
    if (/(gui anh nhieu|anh nhieu lan|spam anh|cung gui anh|cu gui anh|tai sao.*gui.*anh|vi sao.*gui.*anh)/.test(normalized)) {
        return true;
    }
    return false;
}
export function isExplicitImageSampleRequest(text) {
    if (shouldBlockImageSampleSend(text))
        return false;
    const lower = text.toLowerCase();
    if (/(xem mẫu|xem mau|gửi mẫu|gui mau|gửi hình|gui hinh|cho xem mẫu|cho xem mau|muốn xem|muon xem|có hình mẫu|co hinh mau|tham khảo mẫu|tham khao mau)/i.test(lower)) {
        return true;
    }
...
function inferPrimaryImageSampleKey(text) {
    const normalized = normalizeSearchText(text);
    if (!normalized)
        return null;
    const has = (word) => new RegExp(`\\b${word}\\b`).test(normalized);
    const hasAny = (words) => hasAnyWord(normalized, words);
    ...
    const wantsExtensionHairSample = /\bmẫu\s+nối\b|\bxem\s+mẫu\s+nối\b|\bgửi\s+mẫu\s+nối\b|\bhình\s+(?:mẫu\s+)?nối(?:\s+tóc)?\b|\bmối\s+nối\b|\bnối\s+tóc\b/i.test(text);
    if (wantsExtensionHairSample && !/\blông\s*vũ\b/i.test(text) && !/\blong vu\b/i.test(normalized)) {
        return 'noi_toc';
    }
    if (normalized.includes('noi long vu den') ||
    ...
    if (normalized.includes('moi noi long vu'))
        return 'moi_noi_long_vu';
    if (/\blông\s*vũ\b/i.test(text) || normalized.includes('noi long vu') || normalized.includes('long vu')) {
        return 'moi_noi_long_vu';
    }
    if (normalized.includes('noi toc') || normalized.includes('toc noi'))
        return 'noi_toc';
    if (has('phu') && has('bac') && has('tram'))
        return 'phu_bac_mau_tram';
    if (normalized.includes('nhuom phu bac') || (has('phu') && has('bac')))
        return 'nhuom_phu_bac';
    if (isSilverCoverageContext(normalized))
        return 'toc_bac';
    if (normalized.includes('mai thua'))
        return 'mai_thua';
    if (normalized.includes('mai bay'))
        return 'mai_bay';
    if (normalized.includes('mai phap'))
        return 'mai_phap';
    if (normalized.includes('mai ngang'))
        return 'mai_ngang';
    if (has('duoi') && isShort)
        return 'duoi_ngan';
    if (has('duoi') && isLong)
        return 'duoi_dai';
    if (normalized.includes('uon cup'))
        return 'uon_cup';
    if (normalized.includes('uon song') && isShort)
        return 'uon_song_ngan';
    if (normalized.includes('uon song') && isLong)
        return 'uon_song_dai';
    if (hasAny(['hippie', 'hippi', 'hippe']) || normalized.includes('xu mi'))
        return 'uon_hippie';
    if (normalized.includes('xoan tang'))
        return 'uon_xoan_tang';
    ...
    if (has('balayage'))
        return 'mau_balayage';
    if (normalized.includes('baby light') || has('babylight'))
        return 'mau_babylight';
    ...
    if (normalized.includes('mau thoi trang'))
    ...
    if (normalized.includes('mau tram') || normalized.includes('mau toi nhe'))
        return 'mau_tram';
    return null;
}
...
export function slugifyImageSampleLabel(label) {
...
export function imageSampleKeyForLabel(label) {
    const normalized = normalizeSearchText(label);
    if (normalized.includes('moi noi long vu'))
        return 'moi_noi_long_vu';
    if (normalized.includes('noi long vu den'))
        return 'noi_long_vu_den';
    if (normalized.includes('noi toc'))
        return 'noi_toc';
    if (normalized.includes('duoi') && normalized.includes('ngan'))
        return 'duoi_ngan';
    if (normalized.includes('duoi') && normalized.includes('dai'))
        return 'duoi_dai';
    if (normalized.includes('uon cup'))
        return 'uon_cup';
    if (normalized.includes('uon song') && normalized.includes('ngan'))
        return 'uon_song_ngan';
    if (normalized.includes('uon song') && normalized.includes('dai'))
        return 'uon_song_dai';
    ...
    if (normalized.includes('xoan tang'))
        return 'uon_xoan_tang';
    if (normalized.includes('xoan luoi') && normalized.includes('dai'))
        return 'uon_xoan_luoi_dai';
    if (normalized.includes('xoan luoi') && normalized.includes('ngan'))
        return 'uon_xoan_luoi_ngan';
    ...
    if (normalized.includes('mai ngang'))
        return 'mai_ngang';
    if (normalized.includes('phu bac') && normalized.includes('mau tram'))
        return 'phu_bac_mau_tram';
    if (normalized.includes('nhuom phu bac'))
        return 'nhuom_phu_bac';
    if (normalized.includes('toc bac'))
        return 'toc_bac';
    if (normalized.includes('mau tram'))
        return 'mau_tram';
    if (normalized.includes('mau thoi trang'))
        return 'mau_thoi_trang';
    if (normalized.includes('balayage'))
        return 'mau_balayage';
    if (normalized.includes('baby light') || normalized.includes('babylight'))
        return 'mau_babylight';
    if (normalized.includes('sang khong') || normalized.includes('khong can tay'))
        return 'nhuom_sang_khong_tay';
    return slugifyImageSampleLabel(label);
}
export function extractImageSampleUrls(text) {
    return Array.from(new Set(text.match(/(?:https?:\/\/[^\s),\]]+|(?:\.?\/)?images\/samples\/[^\s),\]]+)/g) ?? []));
}
function normalizePublicImageBaseUrl(baseUrl) {
...
export function resolveImageSampleUrl(rawUrl, baseUrl) {
...
export function parseImageSampleGroups(markdown) {
    const groups = [];
...
export function buildImageSampleCatalogPrompt(groups) {
    if (!groups.length)
        return '';
    return [
...
export function mergeContextWithImageSampleCatalog(contextMd, imageSampleGroups) {
...
export function compactLines(text) {
    return text
        .split(/\r?\n/)
...
export function inferImageSampleKeys(text, groups) {
    if (!isExplicitImageSampleRequest(text))
        return [];
    const normalized = normalizeSearchText(text);
    const primaryKey = inferPrimaryImageSampleKey(text);
    if (primaryKey && groups.some((group) => group.key === primaryKey))
        return [primaryKey];
    const keys = [];
...
export function resolveApprovedImageSampleKeys(triggerText, groups) {
    if (!triggerText.trim())
        return [];
    if (shouldBlockImageSampleSend(triggerText))
        return [];
...
export function isSilverCoverageContext(text) {
    const normalized = normalizeSearchText(text);
    return /\b(phu bac|toc bac|bac trang|nhieu bac|hoa bac|nuoi bac)\b/.test(normalized);
}
export function filterImageSampleKeysForContext(keys, contextText) {
    if (!isSilverCoverageContext(contextText))
        return keys;
    return keys.filter((key) => key !== 'mau_tram');
}
export function expandModelImageSampleMarkers(rawText, groups, triggerText, 
...
opts) {
    ...
    for (const [legacyKey, currentKey] of Object.entries(IMAGE_SAMPLE_LEGACY_KEYS)) {
        ...
        if (group)
            groupsByKey.set(legacyKey, group);
    }
    const maxPerGroup = Math.max(1, Math.min(8, opts?.maxImagesPerGroup ?? DEFAULT_MAX_IMAGE_SAMPLES_PER_REPLY));
    ...
    const markerKeys = [];
    let hadMarker = false;
    const textWithoutMarkers = rawText.replace(IMAGE_SAMPLE_MARKER_RE, (_marker, rawKey) => {
        hadMarker = true;
        const key = rawKey.trim().toLowerCase();
        if (groupsByKey.has(key) && !markerKeys.includes(key))
            markerKeys.push(key);
        return '';
    });
    const enforced = opts?.enforceCustomerApprovedKeys;
    /** Khi không nghiêm: key suy được từ khách để ghép vào luồng cũ. */
    const customerKeysFallback = enforced === undefined && !opts?.inferImageKeysFromModelOnly
    ...
    if (enforced !== undefined) {
        ...
        validMarkerKeys = markerKeys.filter((key) => groupsByKey.has(key) &&
    ...
    if (markerKeys.length > 0)
        autoKeys = [];
    else if (enforced !== undefined)
        autoKeys = [];
    else if (opts?.inferImageKeysFromModelOnly)
        autoKeys = [];
    else
        autoKeys = customerKeysFallback;
    const keys = filterImageSampleKeysForContext([...validMarkerKeys, ...autoKeys].filter((key, index, arr) => arr.indexOf(key) === index), contextBlob);
    if (!keys.length) {
    ...
    for (const key of keys) {
        ...
        for (const u of resolvedUrls) {
            ...
            if (!t || seenUrl.has(t))
                continue;
            if (imageUrls.length >= maxPerGroup)
                break;
            seenUrl.add(t);
            imageUrls.push(t);
        }
...
export function injectRealtimeSystemContext(contextMd, now = new Date()) {
    ...
    let out = contextMd;
    out = out.replace(/^- Thời gian hiện tại:.*$/m, `- Thời gian hiện tại: ${timeLabel} (${SALON_TIMEZONE.replace('_', ' ')})`);
    out = out.replace(/^- Lưu ý thời gian:.*$/m, `- Lưu ý thời gian: **Thứ 7 hoặc Chủ nhật** (theo múi giờ VN) → hạn ưu đãi **tuần sau**; **Thứ 2–Thứ 6** → **tuần này**. **Cấm** nói "tuần này" khi hôm nay là T7/CN.\n- Hạn ưu đãi (theo thời gian thực): **${promoDeadline}** — dùng nguyên cụm này khi báo hạn (KNOWLEDGE 2); **tối đa 1 lần / hội thoại** (đọc tin salon đã gửi — đã có thì **cấm** lặp); **cấm** đổi sang "tuần này" nếu block trên ghi tuần sau.`);
    return out;
}
/** Prompt cache Context API — không gắn giờ/phút (tránh tạo cache mới mỗi phút). */
export function buildSalonSystemPromptStatic(contextMd, branch) {
...
export function buildRealtimeContextBlock(now = new Date()) {
    const weekday = new Intl.DateTimeFormat('en-US', { timeZone: SALON_TIMEZONE, weekday: 'short' }).format(now);
    const isSatOrSun = weekday === 'Sat' || weekday === 'Sun';
    const timeLabel = new Intl.DateTimeFormat('vi-VN', {
        timeZone: SALON_TIMEZONE,
...
export function prependRealtimeContextTurns(history) {
...
export function buildSalonSystemPrompt(contextMd, branch) {
...
export function filterPrematureScheduleAskLines(lines, customerHasNamedService) {
 ...
 */
export function ensureAskServiceLineWhenNeeded(lines, customerHasNamedService) {
    if (customerHasNamedService)
        return lines;
    if (lines.some((line) => lineLooksLikeAskService(line)))
        return lines;
...
export function lineLooksLikeBlockedScheduleAsk(line) {
    const n = normalizeScheduleAskCheckText(line);
    if (!n)
        return false;
    return BLOCKED_SCHEDULE_ASK_LINE_RE.some((re) => re.test(n));
}
export function conversationAlreadyUsedBlockedScheduleAsk(messages, isOutbound) {
...
export function filterRepeatedBlockedScheduleAskLines(lines, alreadyAsked) {
    if (!alreadyAsked)
        return lines;
    return lines.filter((line) => !lineLooksLikeBlockedScheduleAsk(line));
}
...
export function lineLooksLikePromoDeadlineLine(line) {
    const n = normalizeScheduleAskCheckText(line);
    if (!n)
        return false;
    return PROMO_DEADLINE_LINE_RE.some((re) => re.test(n));
}
export function conversationAlreadyUsedPromoDeadline(messages, isOutbound) {
 ...
 */
export function filterPromoDeadlineLines(lines, alreadyUsedInConversation) {
    let keptInThisReply = false;
    return lines.filter((line) => {
        if (!lineLooksLikePromoDeadlineLine(line))
            return true;
        if (alreadyUsedInConversation)
            return false;
        if (keptInThisReply)
            return false;
...
function isSalonTuKaWaBinhTanMainPage(normalizedName) {
    if (!/\bsalon\b/.test(normalizedName))
        return false;
    if (!/\b(tu\s*ka\s*wa|tukawa)\b/.test(normalizedName))
        return false;
    if (/(?:^|\s)(?:cn|chi nhanh)\s*#?\s*\d{1,2}\b/.test(normalizedName))
        return false;
    if (matchFanpageBranchKeywords(normalizedName))
        return false;
    if (matchByBranchLabelInPageName(normalizedName))
        return false;
...
function matchFanpageBranchKeywords(normalizedName) {
    const hits = [];
    for (const row of FANPAGE_BRANCH_KEYWORDS) {
        for (const k of row.keys) {
            if (normalizedName.includes(k))
                hits.push({ id: row.id, keyLen: k.length });
        }
    }
    if (!hits.length)
        return null;
...
function matchByBranchLabelInPageName(normalizedName) {
    let best = null;
    for (const b of BRANCH_PAGES) {
        const label = normalizeSearchText(b.name.replace(/^cn\s*\d+\s*[-–]\s*/i, ''));
        if (label.length < 4 || !normalizedName.includes(label))
            continue;
        if (!best || label.length > best.len)
            best = { b, len: label.length };
    }
    return best?.b ?? null;
}
function parseCnNumberFromPageName(pageName) {
    const m = pageName.match(/\bCN\s*#?\s*(\d{1,2})\b/i) ??
        pageName.match(/\bchi\s*nh[aá]nh\s*#?\s*(\d{1,2})\b/i) ??
        pageName.match(/\bCN(\d{1,2})\b/i);
    if (!m)
        return null;
    const n = parseInt(m[1], 10);
    if (!Number.isFinite(n) || n < 1 || n > BRANCH_PAGES.length)
        return null;
    return n;
}
/** Chi nhánh cố định theo tên fanpage (khớp CN / địa danh / tên CN trong page). */
export function inferBranchForFacebookPage(page, _pageOrderIndex = 0) {
    const normalizedName = normalizeSearchText(page.name);
    const cn = parseCnNumberFromPageName(page.name);
    if (cn != null)
        return branchById(cn);
    ...
    if (byKeyword)
        return byKeyword;
    if (isSalonTuKaWaBinhTanMainPage(normalizedName))
        return branchById(2);
    let h = 0;
    for (let i = 0; i < page.id.length; i++)
        h = (Math.imul(31, h) + page.id.charCodeAt(i)) | 0;
