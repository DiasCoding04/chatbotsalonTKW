...
export function estimateContextCacheTokens(text) {
    const t = text.trim();
    if (!t)
        return 0;
    return Math.ceil(t.length / 3.2);
}
export function isContextCacheEligible(systemPrompt) {
    return estimateContextCacheTokens(systemPrompt) >= GEMINI_CONTEXT_CACHE_MIN_TOKENS;
}
export function isMinimumTokenCacheError(message) {
    const m = message.toLowerCase();
    return (m.includes('minimum token count') ||
