...
const VAULT_FILE = process.env.FACEBOOK_TOKEN_VAULT_PATH?.trim() ||
    resolve(DATA_DIR, 'facebook-token-vault.json');
const VAULT_BACKEND = process.env.FACEBOOK_TOKEN_VAULT_BACKEND?.trim().toLowerCase() ||
    process.env.FACEBOOK_STORE_BACKEND?.trim().toLowerCase() ||
    (process.env.K_SERVICE ? 'firestore' : 'file');
const FIRESTORE_PROJECT_ID = process.env.FACEBOOK_TOKEN_VAULT_FIRESTORE_PROJECT_ID?.trim() ||
    process.env.FACEBOOK_STORE_FIRESTORE_PROJECT_ID?.trim() ||
    process.env.VERTEX_AI_PROJECT_ID?.trim() ||
    process.env.GOOGLE_CLOUD_PROJECT?.trim() ||
    '';
const FIRESTORE_DATABASE = process.env.FACEBOOK_TOKEN_VAULT_FIRESTORE_DATABASE?.trim() ||
    process.env.FACEBOOK_STORE_FIRESTORE_DATABASE?.trim() ||
    '(default)';
const FIRESTORE_COLLECTION = process.env.FACEBOOK_TOKEN_VAULT_FIRESTORE_COLLECTION?.trim() ||
    process.env.FACEBOOK_STORE_FIRESTORE_COLLECTION?.trim() ||
    'salon_chat';
const FIRESTORE_DOC_ID = process.env.FACEBOOK_TOKEN_VAULT_FIRESTORE_DOC_ID?.trim() || 'facebook_token_vault';
...
export function vaultFromPageRows(userToken, pages, refresh) {
    const pageTokens = {};
    const pageNames = {};
...
export function applyVaultToProcessEnv(vault) {
...
function resolveVaultReadCacheMs() {
...
let vaultMemCache = null;
export function invalidateFacebookTokenVaultCache() {
    vaultMemCache = null;
}
async function loadFacebookTokenVaultUncached() {
    if (VAULT_BACKEND === 'file') {
        try {
            const raw = await readFile(VAULT_FILE, 'utf8');
            const parsed = JSON.parse(raw);
            if (!parsed.userAccessToken || !parsed.pageTokens)
                return null;
            return {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(raw || `Firestore read vault (${res.status})`);
...
export async function loadFacebookTokenVault() {
...
export async function saveFacebookTokenVault(vault) {
    ...
    const raw = await res.text();
    if (!res.ok)
        throw new Error(raw || `Firestore write vault (${res.status})`);
    vaultMemCache = { vault, at: Date.now() };
}
