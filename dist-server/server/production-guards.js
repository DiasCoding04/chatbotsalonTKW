function isRunningOnGoogleCloud() {
...
export function assertProductionEnv() {
    ...
    if (useVertex) {
        if (!process.env.VERTEX_AI_PROJECT_ID?.trim())
            missing.push('VERTEX_AI_PROJECT_ID');
    ...
    if (!process.env.CONTEXT_EDITOR_TOKEN?.trim())
        missing.push('CONTEXT_EDITOR_TOKEN');
    if (missing.length) {
        throw new Error(`Thiếu biến môi trường bắt buộc khi NODE_ENV=production: ${missing.join(', ')}`);
    }
}
