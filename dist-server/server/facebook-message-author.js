...
const aiOutboundMessageIds = new Map();
function pruneAiOutboundIds() {
    const now = Date.now();
...
export function registerAiOutboundMessageId(messageId) {
    const id = messageId.trim();
    if (!id)
        return;
    aiOutboundMessageIds.set(id, Date.now() + AI_OUTBOUND_TTL_MS);
    if (aiOutboundMessageIds.size > 5000)
        pruneAiOutboundIds();
}
export function isAiOutboundMessageId(messageId) {
    const id = messageId?.trim();
    if (!id)
        return false;
...
export function isStoredAiMessageId(persistedIds, messageId) {
    const id = messageId?.trim();
    if (!id)
        return false;
