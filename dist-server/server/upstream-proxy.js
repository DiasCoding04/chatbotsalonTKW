...
function upstreamTimingEnabled() {
...
function rewriteVertexGeminiPath(pathname) {
...
function rewriteVertexSearch(search) {
    const params = new URLSearchParams(search.startsWith('?') ? search.slice(1) : search);
    params.delete('key');
    const next = params.toString();
    return next ? `?${next}` : '';
}
function rewriteUpstreamSearch(search, injectGeminiKey) {
    if (!injectGeminiKey)
        return search;
...
export function tryProxyUpstream(req, res) {
    const url = req.url ?? '';
    ...
    void (async () => {
        ...
        proxyReq.on('error', (e) => {
            const msg = e instanceof Error ? e.message : String(e);
            if (logTiming) {
                console.error(`[upstream] ${req.method ?? 'GET'} ${upstreamPath} error=${msSince(startedAt).toFixed(0)}ms ${msg}`);
            }
            if (!res.headersSent) {
                res.statusCode = 502;
                res.setHeader('Content-Type', 'application/json; charset=utf-8');
