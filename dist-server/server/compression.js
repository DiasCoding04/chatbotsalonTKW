...
export function negotiateContentEncoding(req) {
    ...
    const tokens = value
        ...
        .map((s) => s.trim())
        .filter(Boolean);
    if (tokens.some((t) => t.startsWith('br')))
        return 'br';
    if (tokens.some((t) => t.startsWith('gzip')))
        return 'gzip';
    return null;
}
/** True nếu MIME type thuộc nhóm nên nén (text, JSON, JS, SVG…). */
export function shouldCompressMime(contentType) {
    if (!contentType)
        return false;
    ...
    if (ct.startsWith('application/json'))
        return true;
    if (ct.startsWith('application/javascript'))
        return true;
...
function applyEncodingHeaders(res, encoding) {
...
export function writeMaybeCompressed(req, res, body) {
...
export function pipeMaybeCompressed(req, res, source) {
    if (req.method === 'HEAD') {
        res.end();
        const maybeDestroy = source.destroy;
