...
function applyEnvBlock(raw: string, mode: 'fill' | 'override'): void {
  for (const line of raw.split(/\r?\n/)) {
    ...
    if (!key) continue
    if (mode === 'fill' && key in process.env) continue
 ...
 */
export function loadEnvFile(_ignoredPath?: string): void {
  const root = process.cwd()
  const chain: Array<{ path: string; mode: 'fill' | 'override' }> = [
