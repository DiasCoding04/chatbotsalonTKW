...
import {
  ...
  IMAGE_SAMPLE_LEGACY_KEYS,
  resolveImageSampleUrl,
  normalizeSearchText,
  parseImageSampleGroups,
  type ImageSampleGroup,
} from '../shared/salon-ai-context.ts'
import { readImageSamplesDocument } from './context-store.ts'
...
type ParentDef = { code: string; title: string; keys: readonly string[] }

/** 6 nút cha → mỗi nhánh ≤ 8 nút con (< giới hạn ~11 của Meta). */
export const IMAGE_CATALOG_PARENTS: ParentDef[] = [
  {
    code: 'JOIN',
    title: 'Nối · lông vũ',
...
export const IMAGE_CATALOG_CHILD_TITLES: Record<string, string> = {
...
function qrTitle(label: string): string {
  const t = label.trim()
  if (t.length <= QR_TITLE_MAX) return t
  return `${t.slice(0, Math.max(0, QR_TITLE_MAX - 1))}…`
}

export function messengerCatalogInviteFromCustomerText(raw: string): boolean {
  const trimmed = raw.trim()
  const n = normalizeSearchText(raw)
  return (
    /\b#\s*anhmau\b|\b#\s*anh\s*mau\b|\b#\s*samp(le)?anh\b|\bmenu\s+danh\s*sach\b/.test(n) ||
...
export function parentPayload(code: string): string {
  return `${MESSENGER_CATALOG_PAYLOAD_PREFIX}P:${code}`
}

export function childPayload(sampleKey: string): string {
  return `${MESSENGER_CATALOG_PAYLOAD_PREFIX}K:${sampleKey}`
}

export function parseCatalogPayload(raw?: string): { kind: 'parent'; code: string } | { kind: 'child'; key: string } | null {
  const p = raw?.trim() ?? ''
  if (!p.startsWith(MESSENGER_CATALOG_PAYLOAD_PREFIX)) return null
...
export function catalogTapDisplayLine(parsed: Exclude<ReturnType<typeof parseCatalogPayload>, null>): string {
...
let groupsCache: { at: number; groups: ImageSampleGroup[] } | null = null
const GROUPS_TTL_MS = 60_000

export async function loadImageSampleGroupsCached(): Promise<ImageSampleGroup[]> {
  const now = Date.now()
  if (groupsCache && now - groupsCache.at < GROUPS_TTL_MS) return groupsCache.groups
...
function groupsWithLegacyAliases(groups: ImageSampleGroup[]): Map<string, ImageSampleGroup> {
  ...
  for (const [legacyKey, currentKey] of Object.entries(IMAGE_SAMPLE_LEGACY_KEYS)) {
    ...
    if (g) map.set(legacyKey, g)
  }
  return map
}

type QuickReplyPayload = { content_type: 'text'; title: string; payload: string }

async function graphPostMessenger(token: string, body: Record<string, unknown>): Promise<{ ok: boolean; err?: string }> {
  ...
  if (!res) return { ok: false, err: 'network' }
  const data = (await res.json()) as GraphSendMessengerResponse | { error?: { message?: string } }
  if (!res.ok) {
    const msg =
...
type GraphSendMessengerResponse = { messaging_type?: string; recipient_id?: string }

async function graphSendQuickReplies(
  token: string,
  recipientPsid: string,
  text: string,
  quick_replies: QuickReplyPayload[],
): Promise<{ ok: boolean; err?: string }> {
  if (!quick_replies.length) return { ok: false, err: 'empty quick_replies' }
...
async function fetchImageBuffer(urlStr: string): Promise<{ buf: Buffer; mime: string } | null> {
...
async function graphSendMessengerText(token: string, recipientPsid: string, text: string): Promise<void> {
...
export async function sendMessengerCatalogParentMenu(
  getToken: (pageId: string) => Promise<string | null>,
  pageId: string,
  recipientPsid: string,
): Promise<void> {
  ...
  const quick_replies: QuickReplyPayload[] = IMAGE_CATALOG_PARENTS.map((p) => ({
    content_type: 'text',
    title: qrTitle(p.title),
    payload: parentPayload(p.code),
  }))
  const r = await graphSendQuickReplies(
    token,
    recipientPsid,
    'Chị chọn nhóm ảnh mẫu (bấm 1 ô bên dưới để có thêm mẫu con) ạ.',
    quick_replies,
  )
  if (!r.ok) console.warn('[catalog-menu] send parent QR failed:', r.err)
}

export async function sendMessengerCatalogChildMenu(
  getToken: (pageId: string) => Promise<string | null>,
  pageId: string,
  recipientPsid: string,
  parentCode: string,
  ...
  const quick_replies: QuickReplyPayload[] = def.keys.map((key) => ({
    content_type: 'text',
  ...
  const r = await graphSendQuickReplies(
    token,
    recipientPsid,
    `Chọn kiểu trong nhóm "${def.title}":`,
    quick_replies,
  )
  if (!r.ok) console.warn('[catalog-menu] send child QR failed:', r.err)
}

export async function sendMessengerCatalogSampleImages(
  getToken: (pageId: string) => Promise<string | null>,
  pageId: string,
  recipientPsid: string,
  sampleKey: string,
  ...
  if (!/^https?:\/\//i.test(baseUrl)) {
    ...
    await graphSendMessengerText(
      token,
      recipientPsid,
      'Salon đang thiếu IMAGE_SAMPLES_BASE_URL nên không gửi ảnh tự động được ạ. Chị tham khảo mẫu trên tin nhắn trước hoặc báo stylist ạ.',
    )
    return
  }
  ...
  let group =
    map.get(sampleKey.toLowerCase()) ??
    [...map.values()].find((g) => g.key.replace(/_/g, '') === sampleKey.replace(/_/g, ''))

  /** Accept legacy filenames */
  const legacyCanon = IMAGE_SAMPLE_LEGACY_KEYS[sampleKey as keyof typeof IMAGE_SAMPLE_LEGACY_KEYS]
  if (legacyCanon) group = map.get(legacyCanon) ?? group
  ...
  await graphSendMessengerText(
    token,
    recipientPsid,
    `${label} — em gửi vài ảnh mẫu gần với catalogue ạ (tối đa ${MAX_SAMPLES_PER_REPLY} ảnh).`,
  )
  let sent = 0
  ...
  for (const absolute of urls) {
    ...
    if (!fetched || fetched.buf.length > 8 * 1024 * 1024) continue
