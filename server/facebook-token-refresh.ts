...
export type FacebookPageTokenRow = { id: string; name: string; access_token: string }

import {
  applyVaultToProcessEnv,
  loadFacebookTokenVault,
  saveFacebookTokenVault,
  vaultFromPageRows,
} from './facebook-token-vault.ts'
...
function shouldExchangeUserToken(): boolean {
  if (envFlag('FACEBOOK_SKIP_USER_TOKEN_EXCHANGE')) return false
  if (envFlag('FACEBOOK_EXCHANGE_USER_TOKEN')) return true
  return true
}

export function facebookTokenAutoRefreshEnabled(): boolean {
  if (process.env.FACEBOOK_TOKEN_REFRESH_ON_STARTUP?.trim() === '0') return false
...
export async function fetchAllPageTokensFromUserToken(
  userToken: string,
): Promise<FacebookPageTokenRow[]> {
  const pages: FacebookPageTokenRow[] = []
...
export function applyPageTokensToProcessEnv(pages: FacebookPageTokenRow[]): void {
...
export function pageTokenMapFromRows(pages: FacebookPageTokenRow[]): Map<string, string> {
  return new Map(pages.map((p) => [p.id, p.access_token]))
}

export async function refreshFacebookPageTokensFromEnvUserToken(): Promise<{
  ok: boolean
  pageCount: number
  message: string
}> {
  ...
  try {
    ...
    if (!pages.length) {
      ...
      try {
        await saveFacebookTokenVault(
          vaultFromPageRows(userToken, [], { ok: false, message: fail.message }),
        )
      } catch {
    ...
    try {
      await saveFacebookTokenVault(
        vaultFromPageRows(userToken, pages, { ok: true, message: result.message }),
      )
    } catch (e) {
      console.warn(
        '[facebook] Không lưu token vault:',
...
export async function bootstrapFacebookTokensFromVault(): Promise<number> {
  const vault = await loadFacebookTokenVault()
  if (!vault?.pageTokens || !Object.keys(vault.pageTokens).length) return 0
  const n = applyVaultToProcessEnv(vault)
  if (n > 0) {
    console.log(
      `[facebook] Đã nạp ${n} page token từ vault (${vault.updatedAt}) — ${vault.lastRefreshMessage ?? ''}`,
    )
  }
  return n
}
